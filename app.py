...
TEAM = {
    "group_no": "G-__",
    "members": [
        {"name": "Student Name 1", "enrollment": "________"},
        {"name": "Student Name 2", "enrollment": "________"},
        {"name": "Student Name 3", "enrollment": "________"},
        {"name": "Student Name 4", "enrollment": "________"},
    ],
...
st.markdown(
    f"""
    <style>
    .stApp {{ background-color: {BG}; color: #E8EEF6; }}
    ...
    .stButton>button {{
        background-color: {ORANGE}; color: {NAVY}; font-weight: 700; border: none;
    }}
    thead tr th {{ background-color: {STEEL} !important; color: white !important; }}
    </style>
    """,
...
# ====================================================================================
    ...
    lathe_html = f"""
    <div id="lathe-container" style="width:100%;height:560px;border-radius:10px;overflow:hidden;background:#0E1626;position:relative;">
      <div id="tooltip" style="position:absolute;top:10px;left:10px;background:#152238;color:#FF7A00;
           padding:6px 12px;border-radius:6px;font-family:sans-serif;font-size:14px;border:1px solid #3A506B;">
           Click a part of the lathe
      </div>
      <div style="position:absolute;top:10px;right:10px;">
        <button id="simBtn" style="background:#FF7A00;color:#0B1F3A;font-weight:700;border:none;
    ...
    (function() {{
      ...
      const steel = new THREE.MeshStandardMaterial({{color: 0x3A506B, metalness:0.6, roughness:0.4}});
      const navy  = new THREE.MeshStandardMaterial({{color: 0x0B1F3A, metalness:0.4, roughness:0.6}});
      const orange= new THREE.MeshStandardMaterial({{color: 0xFF7A00, metalness:0.5, roughness:0.3}});
      const bronze= new THREE.MeshStandardMaterial({{color: 0xC87941, metalness:0.7, roughness:0.3}});
...
# ====================================================================================
    ...
    if not st.session_state["computed"] or not st.session_state["rows"]:
        ...
        anim_html = f"""
        <div style="background:#0E1626;border-radius:10px;padding:10px;">
        <canvas id="turnCanvas" width="760" height="260" style="background:#152238;border-radius:8px;"></canvas>
        <div style="margin-top:8px;">
          <button id="playBtn" style="background:#FF7A00;color:#0B1F3A;font-weight:700;border:none;padding:6px 14px;border-radius:6px;">▶ Play</button>
          <button id="pauseBtn" style="background:#3A506B;color:white;font-weight:700;border:none;padding:6px 14px;border-radius:6px;">⏸ Pause</button>
          <button id="restartBtn" style="background:#3A506B;color:white;font-weight:700;border:none;padding:6px 14px;border-radius:6px;">⟲ Restart</button>
          <span style="color:#8FA6C1;margin-left:10px;">Cross-section view — tool sweeps left→right removing stock</span>
        </div>
        </div>
        <script>
        (function() {{
        ...
        with g1:
            ...
            ax1.plot(start_d, rpms, marker="o", color="#FF7A00")
        ...
        with g2:
            ...
            ax2.bar(passes, times, color="#3A506B", label="Time per pass")
            ax2.plot(passes, cum_times, marker="o", color="#FF7A00", label="Cumulative time")
        ...
        ax3.plot(passes, start_d, marker="o", label="Start Diameter", color="#FF7A00")
        ax3.plot(passes, end_d, marker="s", label="End Diameter", color="#8FA6C1")
